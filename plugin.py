...
class BasePlugin:
    ...
    def Presence(self):
        ...
        if Parameters["Mode4"] == "":
            Domoticz.Debug("No presence detector found...")
            self.Presencemode = False
            self.Presence = False
        else :
            ...
            for device in devicesAPI["result"]:  # parse the presenceDT device
                ...
                if idx in self.DTpresence:  # this is one of our DT
                    if "Status" in device:
                        ...
                        if self.presencechangedtime + timedelta(minutes=5) <= now:
                            Domoticz.Debug("Presence is now ACTIVE !")
                            self.Presence = True
                        elif self.presencechangedtime + timedelta(minutes=30) <= now:
                            Domoticz.Debug("Presence is now INACTIVE !")
                            self.Presence = False


                        self.endpresence = datetime.now() + timedelta(minutes=30)
