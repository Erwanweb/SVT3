...
# AC Smart Virtual Thermostat for TRV for Domoticz
# Author: Erwanweb,
# Version:    0.0.1: alpha
# Version:    0.0.2: beta
# Version:    0.1.1: Correction for reducted temp if no presence
# Version:    0.1.2: Correction for control of TRV setpoint
# Version:    0.1.3: Correction for control of TRV setpoint when off
# Version:    2.0.1: Adding smart delay for zigbee messages
# Version:    2.0.2: API request timeout, presence state held internally, code cleanup

"""
<plugin key="SVT3" name="AC Smart Virtual Thermostat for TRV" author="Erwanweb" version="2.0.2" externallink="https://github.com/Erwanweb/SVT3.git">
    <description>
        <h2>Smart Virtual Thermostat for TRV V2.0.2</h2><br/>
...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.DTtempo = now - timedelta(seconds=40)
        ...
        if Parameters["Mode4"] == "":
            ...
            if devicesAPI:
                for device in devicesAPI["result"]:  # parse the presence/motion sensors (DT) device
                    idx = int(device["idx"])
                    if idx not in self.DTpresence:  # this is not one of our DT
                        continue

                    # skip the DT while it is in the temporary exclusion list
                    if idx in self.DTexcludedUntil:
                        if now < self.DTexcludedUntil[idx]:
                            Domoticz.Debug("DT idx {} still excluded until {}".format(
                                idx, self.DTexcludedUntil[idx]))
                            continue
                        else:
                            del self.DTexcludedUntil[idx]  # back in the game

                    # check the DT is not timed out (dummies never report a timeout)
                    skip = False
                    dummy = True
                        ...
                        if device.get("HaveTimeout", False):
                            ...
                            if last_update_str:
                                try:
                                    ...
                                    Domoticz.Error("LastUpdate parsing error for '{}': {}".format(device["Name"], e))
                                    skip = True

                    if skip:
                        self.DTexcludedUntil[idx] = now + timedelta(minutes=15)
                    ...
                    if "Status" in device:
                        ...
                        Domoticz.Debug("{} switch idx '{}' - '{}' currently is '{}'".format(
                            "Dummy" if dummy else "DT", idx, device["Name"], device["Status"]))
                        if device["Status"] == "On":
                            self.DTtempo = datetime.now()
                    else:
                        Domoticz.Error("Device with idx '{}' named '{}' does not seem to be a DT !".format(
                            idx, device["Name"]))
            ...
            if len(PresenceDT) == 0:
                ...
                self.Presence = False
                self.PresenceSensor = False
                self.PresenceTH = True  # fail safe : confort setpoint when the DT cannot be trusted
                if Devices[8].nValue == 1:
                    Devices[8].Update(nValue = 0,sValue = Devices[8].sValue)
                return
    ...
    def readTemps(self):
        ...
        if devicesAPI:
            for device in devicesAPI["result"]:
                ...
                if idx in self.InTempSensors:
                    target, kind = listintemps, "Temperature"
                elif idx in self.TRVTempSensors:
                    target, kind = listtrvtemps, "TRV Temp"
                else:
                    continue

                # skip the sensor while it is in the temporary exclusion list
                if idx in self.TempExcludedUntil:
                    if now < self.TempExcludedUntil[idx]:
                        Domoticz.Debug("Temp sensor idx {} still excluded until {}".format(
                            idx, self.TempExcludedUntil[idx]))
                        continue
                    else:
                        del self.TempExcludedUntil[idx]  # back in the game

                # check the sensor is not timed out (dummies never report a timeout)
                skip = False
                if device.get("HardwareName") != "Dummies":
                    if device.get("HaveTimeout", False):
                        skip = True
                ...
                if skip:
                    self.TempExcludedUntil[idx] = now + timedelta(minutes=15)
                    Domoticz.Error("Device with idx '{}' named '{}' is TimedOut !".format(idx, device["Name"]))
                    continue

                # valid sensor
                if "Temp" not in device:
                    Domoticz.Error("device: {}-{} is not a {} sensor".format(
                        device["idx"], device["Name"], kind))
                    continue
                Domoticz.Debug("device: {}-{} = {}".format(device["idx"], device["Name"], device["Temp"]))
                target.append(device["Temp"])
        ...
        if nb_in > 0:
            ...
            if self.intemperror:
                ...
                self.intemperror = False
                self.WriteLog("Inside Temperature reading is now valid again: Resuming normal operation", "Normal")
...
def DomoticzAPI(APICall):
    ...
    try:
        ...
        Domoticz.Error(f"Timeout (10s) calling '{url}'")
