...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 8 not in Devices:
            Domoticz.Device(Name="Presence sensor", Unit=8, TypeName="Switch", Image=9, Used=1).Create()
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if Unit in (1, 2, 4, 5, 8): # force recalculation if control or mode or a setpoint changed
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
            ...
            elif self.Presencemode:  # we check the presence mode and if active or not
                if self.Presence:
                   if self.presencechangedtime + timedelta(minutes = 1) <= now:
                    ...
                    if self.presencechangedtime + timedelta(minutes = 3) <= now:
                        ...
                        Domoticz.Debug("Presence is now ACTIVE but in timer OFF !")
    ...
    def PresenceDetection(self):
        ...
        if Parameters["Mode4"] == "":
            ...
            if len(PresenceDT) == 0:
               ...
               self.Presence = False
               self.PresenceTH = False
               Devices[8].Update(nValue = 0,sValue = Devices[7].sValue)
               return
