        ...
        if self.RefreshAndActTime + timedelta(seconds=10) <= now:
            ...
            for idx in self.Heaters:
                ...
                if abs(current_sp - self.TRVsetpoint) > 0.05:
                    ...
                    Domoticz.Debug("TRV idx {} already at setpoint {}, no update".format(idx, current_sp))
    ...
    def readTemps(self):
        ...
        if devicesAPI:
            for device in devicesAPI["result"]:
                ...
                # TRV Temp
                elif idx in self.InTempSensors:
                    # Ignorer temporairement s'il est dans la liste d'exclusion
                    if idx in self.TempExcludedUntil:
                        if datetime.now() < self.TempExcludedUntil[idx]:
                            Domoticz.Debug(
        ...
        # calculate the average Room temperature
        nbtemps = len(listtrvtemps)
        if nbtemps > 0:
            self.intemp = round(sum(listintemps) / nbtemps, 1)
            Devices[6].Update(nValue = 0,sValue = str(self.intemp),TimedOut = False)  # update the dummy device showing the current thermostat temp
