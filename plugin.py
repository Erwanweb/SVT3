...
"""
"""
<plugin key="SVT3" name="AC Smart Virtual Th for TRV" author="logread" version="0.4.4" wikilink="https://www.domoticz.com/wiki/Plugins/Smart_Virtual_Thermostat.html" externallink="https://github.com/999LV/SmartVirtualThermostat.git">
    <description>
...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
            ...
            if Parameters["Mode4"] == "":
                if Parameters["Mode4"] == "":
                    Domoticz.Debug("presence detection = NO...")

                else:
                    self.Presencemode = True
                    Domoticz.Debug("presence detection = YES...")
                    self.PresenceDetection()

            elif self.forced:  # thermostat setting was just changed from "forced" so we kill the forced mode
                self.forced = False
                self.endheat = now
                ...
                # make sure there is at min one presence detector and active the presence mode
    ...
    def switchHeat(self, switch):
        ...
        # flip on / off the TRV as needed by using max and min setpoint
        self.heat = switch
        command = "28" if switch else "17"
        Domoticz.Debug("Heating '{}'".format(command))
        for idx in self.Heaters:
            DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint={}".format(idx,command))
        if switch:
            Domoticz.Debug("End Heat time = " + str(self.endheat))
    ...
    def PresenceDetection(self):
        ...
        if devicesAPI:
            for device in devicesAPI["result"]:  # parse the presenceDT device
                idx = int(device["idx"])
                if idx in self.DTpresence:  # this is one of our DT
                    if "Status" in device:
                        PresenceDT[idx] = True if device["Status"] == "On" else False
                        Domoticz.Debug("PresenceDT switch {} currently is '{}'".format(idx,device["Status"]))
                    else:
        ...
        for idx in self.DTpresence:
            if PresenceDT[idx] == "On":
                ...
                if self.presencechangedtime + timedelta(minutes = 3) <= now:
                    Domoticz.Debug("Presence is now INACTIVE !")
                    self.Presence = False
                else:
                    if self.presencechangedtime + timedelta(minutes = 1) <= now and PresenceDT[idx] == "On":
                        Domoticz.Debug("Presence is now ACTIVE !")
                        self.Presence = True
                    else:
                        self.Presence = False
                        Domoticz.Debug("Presence is INACTIVE because lower than 1 minute !")
            else :
                Domoticz.Debug("No presence detected...")
