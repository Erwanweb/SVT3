    ...
    def PresenceDetection(self):
        ...
        if Parameters["Mode4"] == "":
            ...
            if devicesAPI:
                for device in devicesAPI["result"]:  # parse the presence/motion sensors (DT) device
                    ...
                    if idx in self.DTpresence:  # this is one of our DT
                        if "Status" in device:
                            PresenceDT1[idx] = True if device["Status"] == "On" else False
                            PresenceDT2[idx] = True if device["Status"] == "On" else False
            ...
            for idx in self.DTpresence:
                if PresenceDT1[idx] or PresenceDT2[idx]:
