...
class BasePlugin:
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
            ...
            if self.forced:  # thermostat setting was just changed from "forced" so we kill the forced mode
                ...
                if Devices[2].sValue == "10":
                    if self.Presencemode and self.PresenceTH:
                       self.setpoint = float(Devices[4].sValue)
                    elif self.Presencemode and not self.PresenceTH:
    ...
    def PresenceDetection(self):
        ...
        if Parameters["Mode4"] == "":
            ...
            elif not self.Presence and self.PresenceTH:
                Domoticz.Debug("Presence is now ACTIVE but in timer OFF !")

            elif not self.Presence and self.presencechangedtime + timedelta(minutes = 3) <= now:
