...
<plugin key="SVT3" name="AC Smart Virtual Thermostat for TRV" author="Erwanweb" version="0.1.3" externallink="https://github.com/Erwanweb/SVT3.git">
    <description>
        <h2>Smart Virtual Thermostat for TRV V2.0.1</h2><br/>
        V.0.1.3<br/>
...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.RefreshAndActTime + timedelta(seconde=10) <= now:
            # reset at now the RefreshAndActTime
            self.RefreshAndActTime = datetime.now()
            ...
            #Self.TRVsetpoint = round(self.TRVsetpoint, 1) # on arrondi a 1 chiffre apres la virgule
            ...
            for idx in self.Heaters:
                try:
                    ...
                    Domoticz.Error("Heater idx {} not found in Devices".format(idx))
                    continue
                # récupération du setpoint actuel du TRV
                try:
                    current_sp = round(dev.sValue)
                except (ValueError, AttributeError):
                    Domoticz.Error("Heater idx {} has invalid sValue: '{}'".format(idx, getattr(dev, "sValue", None)))
                    continue
                # on compare avec une petite tolérance pour éviter le spam à cause des flottants
