...
<plugin key="SVT3" name="AC Smart Virtual Thermostat for TRV" author="Erwanweb" version="0.1.2" externallink="https://github.com/Erwanweb/SVT3.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Calculation cycle, Minimum Heating time per cycle, Pause On delay, Pause Off delay, Forced mode duration (all in minutes) reduc jour, reduc nuit (both in tenth of degre)" width="200px" required="true" default="30,0,2,1,60,5,60,10,20"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params) == 9:
            ...
            self.reducjour = CheckParam("reduit jour",params[7],10)
            self.reducnuit = CheckParam("reduit nuit",params[8],20)
        else:
        ...
        if Devices[1].sValue == "0":  # Thermostat is off
            ...
            if self.forced or self.switchHeat:  # thermostat setting was just changed so we kill the heating
                ...
                for idx in self.Heaters:
                    DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint=7".format(idx))
            ...
            Domoticz.Debug("Thermostat is on FORCED mode")
            ...
            if self.forced:  # thermostat setting was just changed from "forced" so we kill the forced mode
                ...
                if self.pauserequestchangedtime + timedelta(minutes=self.pauseondelay) <= now:
                    ...
                    for idx in self.Heaters:
                        DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint=5".format(idx))
                    if not Devices[7].nValue == 0:
                        Devices[7].Update(nValue = 0,sValue = Devices[7].sValue)
                ...
                if Devices[2].sValue == "10":  # Mode Auto
                    if self.PresenceTH:
                        ...
                        Domoticz.Debug("AUTO Mode - used setpoint is NORMAL : " + str(self.setpoint))
                        self.TRVsetpoint = self.setpoint + (self.intemp - self.TRVtemp)  # correction of TRV setpoint using difference between real indoor temp and mesured trv temp.
                        Domoticz.Debug("TRV Calculded setpoint is : " + str(self.TRVsetpoint))
                        for idx in self.Heaters:
                            DomoticzAPI(
                                "type=command&param=setsetpoint&idx={}&setpoint={}".format(idx,self.TRVsetpoint))
                    else:
                        self.setpoint = (float(Devices[4].sValue) - ((self.reducjour) / 10))
                        Domoticz.Debug("AUTO Mode - used setpoint is reducted 1 : " + str(self.setpoint))
                        self.TRVsetpoint = self.setpoint + (self.intemp - self.TRVtemp)  # correction of TRV setpoint using difference between real indoor temp and mesured trv temp.
                        Domoticz.Debug("TRV Calculded setpoint is : " + str(self.TRVsetpoint))
                        for idx in self.Heaters:
                            DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint={}".format(idx,self.TRVsetpoint))
                    ...
                    Domoticz.Debug("ECO Mode - used setpoint is ECO : " + str(self.setpoint))
                    self.TRVsetpoint = self.setpoint + (
                                self.intemp - self.TRVtemp)  # correction of TRV setpoint using difference between real indoor temp and mesured trv temp.
                    Domoticz.Debug("TRV Calculded setpoint is : " + str(self.TRVsetpoint))
                    for idx in self.Heaters:
                        DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint={}".format(idx,self.TRVsetpoint))
                    ...
                    Domoticz.Debug("VACATION Mode - used setpoint is VACATION : " + str(self.setpoint))
                    Domoticz.Debug("TRV Calculded setpoint is : 15")
                    for idx in self.Heaters:
                        DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint=15".format(idx))
